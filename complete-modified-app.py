...
def process_image(img_path: str) -> Image.Image:
    """Helper function to load and process image files"""
    try:
...
@torch.no_grad()
def stream_chat(input_files: list, caption_type: str, caption_length: str | int, extra_options: list[str], name_input: str, custom_prompt: str) -> tuple[str, dict]:
    ...
    for file in input_files:
        try:
            ...
            image = process_image(file.name)
            if image is None:
                continue
                
            # Get the original filename without extension
            img_filename = os.path.splitext(os.path.basename(file.name))[0]
            ...
            results[os.path.splitext(os.path.basename(file.name))[0]] = f"Error: {str(e)}"
